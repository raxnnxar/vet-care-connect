...
/**
 * Fetches all appointments for a specific groomer
 */
export const getGroomingAppointments = async (providerId: string): Promise<Appointment[]> => {
  try {
    ...
    console.error('Unexpected error in getGroomingAppointments:', err);
    return [];
  }
};

/**
 * Fetches appointments for a specific groomer on a specific date
 */
export const getGroomingAppointmentsByDate = async (providerId: string, date: Date): Promise<Appointment[]> => {
  try {
    ...
    if (error) {
      console.error('Error fetching grooming appointments by date:', error);
      throw error;
    }

    console.log('All appointments for groomer:', data);
    ...
    console.error('Unexpected error in getGroomingAppointmentsByDate:', err);
    return [];
  }
};

/**
 * Get dates with appointments for a specific groomer in a date range
 */
export const getGroomingAppointmentDates = async (providerId: string, startDate: Date, endDate: Date): Promise<Date[]> => {
  try {
    const { data, error } = await supabase
      .from('appointments')
    ...
    if (error) {
      console.error('Error fetching grooming appointment dates:', error);
      throw error;
    }
    ...
    console.error('Unexpected error in getGroomingAppointmentDates:', err);
    return [];
  }
};
