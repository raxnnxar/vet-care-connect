...
interface VaccinationFormProps {
  petId: string;
  existingRecord?: VaccinationRecord;
  onClose: () => void;
...
const VaccinationForm: React.FC<VaccinationFormProps> = ({
  ...
  useEffect(() => {
    if (existingRecord) {
      setFormData({
        vaccine_name: existingRecord.vaccine_name || '',
        application_date: existingRecord.application_date || '',
        next_due_date: existingRecord.next_due_date || '',
        manufacturer: existingRecord.manufacturer || '',
        lot_number: existingRecord.lot_number || '',
        lot_expiry_date: existingRecord.lot_expiry_date || '',
        anatomical_site: existingRecord.anatomical_site || '',
        route: existingRecord.route || '',
        notes: existingRecord.notes || '',
        needs_booster: existingRecord.needs_booster || false
      });
    }
  }, [existingRecord]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast.success(existingRecord ? 'Vacuna actualizada exitosamente' : 'Vacuna registrada exitosamente');
      onVaccinationAdded();
    } catch (error) {
      console.error('Error saving vaccination:', error);
      toast.error(existingRecord ? 'Error al actualizar la vacuna' : 'Error al registrar la vacuna');
    } finally {
      setIsLoading(false);
    }
  };

  const isDialog = !existingRecord;
  ...
  const formContent = (
    <form onSubmit={handleSubmit} className="space-y-4">
      ...
      <div className="flex justify-end gap-3 pt-4">
        ...
        >
          {isLoading ? 'Guardando...' : (existingRecord ? 'Actualizar vacuna' : 'Registrar vacuna')}
        </Button>
      </div>
    </form>
  );
