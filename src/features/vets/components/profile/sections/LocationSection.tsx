...
const LocationSection: React.FC<LocationSectionProps> = ({
  ...
  useEffect(() => {
    ...
    const defaultLat = editedCoordinates?.lat || clinicLatitude || 19.4326;
    const defaultLng = editedCoordinates?.lng || clinicLongitude || -99.1332;
  ...
  const reverseGeocode = async (lat: number, lng: number) => {
    try {
      ...
      if (data.features && data.features.length > 0) {
        setEditedAddress(data.features[0].place_name);
      }
    } catch (error) {
          ...
          {clinicAddress ? (
            <div className="flex items-start space-x-3">
              ...
              <div className="flex-1">
                <p className="text-gray-700">{clinicAddress}</p>
                {clinicLatitude && clinicLongitude && (
                  <p className="text-sm text-gray-500 mt-1">
                    Coordenadas: {clinicLatitude.toFixed(6)}, {clinicLongitude.toFixed(6)}
                  </p>
                )}
              </div>
            </div>
